...
class AnnotatorId(Resource):
    ...
    def get(self, image_id):
        ...
        if dataset is None:
            return (
                {"success": False, "message": "Could not find associated dataset"},
                400,
            )
        ...
        pre = (
            images.filter(file_name__lt=image.file_name).order_by("-file_name").first()
        )
        ...
        for category in categories:
            ...
            annotations = (
                AnnotationModel.objects(
                    image_id=image_id, category_id=category_id, deleted=False
                )
                .exclude("events")
                .all()
            )
